...
export function validateSafetyInput(value: unknown): ValidationResult<SafetyInput> {
  ...
  if (typeof body.profileId !== "string" || !UUID_PATTERN.test(body.profileId)) {
    return { ok: false, error: { code: "INVALID_REQUEST", message: "profileId must be a valid UUID." } };
  }
  if (body.action === "block") return { ok: true, value: { action: "block", profileId: body.profileId } };
  if (body.action !== "report" || typeof body.reason !== "string" || !REPORT_REASONS.has(body.reason)) {
    return { ok: false, error: { code: "INVALID_REQUEST", message: "Choose a valid safety action and report reason." } };
  }
  const details = typeof body.details === "string" ? body.details.trim() : "";
  if (details.length > 500) {
    return { ok: false, error: { code: "INVALID_REQUEST", message: "Report details must be 500 characters or fewer." } };
  }
  return {
    ok: true,
    value: { action: "report", profileId: body.profileId, reason: body.reason, details: details || null },
  };
}
...
function configuration(): ValidationResult<{ baseUrl: string; apiKey: string }> {
  ...
  if (!baseUrl || !apiKey) {
    return { ok: false, error: { code: "SERVICE_UNAVAILABLE", message: "This safety action is temporarily unavailable." } };
  }
...
async function handleRequest(request: Request): Promise<Response> {
  ...
  if (!accessToken) {
    return jsonResponse({ code: "AUTH_REQUIRED", message: "Sign in to use safety controls." }, 401, origin, allowedOrigins);
  }
  ...
  if (currentProfileId === input.value.profileId) {
    return jsonResponse({ code: "INVALID_TARGET", message: "You cannot take this action on your own profile." }, 400, origin, allowedOrigins);
  }
  ...
  if (targetError || !targetData) {
    return jsonResponse({ code: "PROFILE_NOT_FOUND", message: "That profile was not found." }, 404, origin, allowedOrigins);
  }
  ...
  if (input.value.action === "report") {
    ...
    if (reportError) {
      return jsonResponse({ code: "INTERNAL_ERROR", message: "Your report could not be submitted." }, 500, origin, allowedOrigins);
    }
    ...
    return jsonResponse({ action: "report", profileId: input.value.profileId, submitted: true }, 200, origin, allowedOrigins);
  }
  ...
  if (blockError && !isUniqueViolation(blockError)) {
    return jsonResponse({ code: "INTERNAL_ERROR", message: "That profile could not be hidden." }, 500, origin, allowedOrigins);
  }
  ...
  return jsonResponse({ action: "block", profileId: input.value.profileId, hidden: true }, 200, origin, allowedOrigins);
}
...
export async function handler(request: Request): Promise<Response> {
  ...
  try {
    ...
    return jsonResponse({ code: "INTERNAL_ERROR", message: "This safety action is temporarily unavailable." }, 500, origin, allowedOrigins);
  }
}
